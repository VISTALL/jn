...
bool SunJVMDLL::instanciate()
{
  ...
  //findBootClass = (FindClassFromBootLoader_t *)GetProcAddress(m_vmlib, "JVM_FindClassFromBootLoader");
        
  if ((CreateJavaVM == NULL) || (GetDefaultJavaVMInitArgs == NULL) /*|| findBootClass == NULL*/)
    {
...
bool SunJVMDLL::setupVM12DLL(CreateJavaVM_t CreateJavaVM, GetDefaultJavaVMInitArgs_t GetDefaultJavaVMInitArgs)
{
  ...
  vm_args.nOptions = JVMOptions::getInstance().getSize();
  vm_args.options = JVMOptions::getInstance().toJavaVMOptions();
  ...
  if(findClass(EXE_MAKER_MAIN_CLASS) == NULL)
  {
	  MessageBox(0, "Not find main class", EXE_MAKER_ERROR_CAPTION, MB_ICONERROR);
	  m_statusCode = SunJVMDLL::JVM_CANT_USE_VM;
	  ExitProcess(-1);
	  return false;
  }
...
/*jclass SunJVMDLL::findBootStrapClass(const char *classname)
{
