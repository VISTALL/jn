...
vector<string> FileUtils::recursiveSearch(const string& pathdir, const string& pattern, const bool recursive)
{
	...
	if(recursive)
	{
		...
		do 
		{
			...
			if ((foundpath != ".") && (foundpath != "..") && ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0))
			{
				...
				vector<string> tres = FileUtils::recursiveSearch(npath, pattern, recursive);
