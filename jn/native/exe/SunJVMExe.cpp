...
std::string SunJVMExe::lookUpExecutable(bool useconsole)
{
  ...
  if (useconsole)
	...
	{
	  std::vector<std::string> javas = FileUtils::recursiveSearch(m_jrehome, "java.exe", true);
	  DEBUG("REC: " + StringUtils::toString(javas));

	  if (javas.size() == 0)
	    javas = FileUtils::recursiveSearch(m_jrehome, "jre.exe", true);
	...
	{
	  std::vector<std::string> javas = FileUtils::recursiveSearch(m_jrehome, "javaw.exe", true);

	  DEBUG("REC: " + StringUtils::toString(javas));

	  if (javas.size() == 0)
	    javas = FileUtils::recursiveSearch(m_jrehome, "jrew.exe", true);
...
std::string SunJVMExe::getClassPath(bool full)
{
  ...
  if (full)
	...
	{
	  vector<string> cpzips = FileUtils::recursiveSearch(home, "*.zip", true);
	  cp.insert(cp.end(), cpzips.begin(), cpzips.end());
	  vector<string> cpjars = FileUtils::recursiveSearch(home, "*.jar", true);
